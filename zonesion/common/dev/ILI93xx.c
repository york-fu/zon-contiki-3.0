...
#ifdef HEALTHYBAND
  if (st) {
    GPIO_SetBits(GPIOD, GPIO_Pin_3); //开启背光
  } else {
...
void BLInit(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;//PD2 í?íìê?3?,????±31a
  GPIO_Init(GPIOC, &GPIO_InitStructure);//3?ê??ˉ //PB15 í?íìê?3?,????±31a
  GPIO_ResetBits(GPIOC, GPIO_Pin_5);
  clock_delay_ms(10);
  GPIO_SetBits(GPIOC, GPIO_Pin_5);

  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE);
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;//PD2 í?íìê?3?,????±31a
  GPIO_Init(GPIOD, &GPIO_InitStructure);//3?ê??ˉ //PB15 í?íìê?3?,????±31a
#else
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE);
...
void LCD_Clear(int x1,int y1,int x2,int y2,uint16_t color)
{
  for(int i=x1;i<=x2;i++)
    for(int j=y1;j<y2;j++){
#ifndef SCREEN_ORIENTATION_LANDSCAPE
      ILI93xx_WriteReg(0x20, j);                                //Y轴
      ILI93xx_WriteReg(0x21, i);                                //X轴       //将“光标”移动到（i,j）
#else
      ILI93xx_WriteReg(0x20, i);                                //Y轴
      ILI93xx_WriteReg(0x21, j);                                //X轴
#endif
      ILI93xx_REG = 0x22;                                       //显示命令
      ILI93xx_DAT = color;
    }
}