...
static LoRaMacStatus_t AddMacCommand( uint8_t cmd, uint8_t p1, uint8_t p2 )
{
    ...
    switch( cmd )
    {
        ...
        case MOTE_MAC_DEVICE_MODEL_IND:
            if( MacCommandsBufferIndex < ( bufLen - 1 ) )
            {
                MacCommandsBuffer[MacCommandsBufferIndex++] = cmd;
                // mdevice mode 
    ...
    while( macIndex < commandsSize )
    {
        ...
        switch( payload[macIndex++] )
        {
            ...
            case SRV_MAC_DL_CHANNEL_REQ:
                ...
                break;
            case MOTE_MAC_DEVICE_MODEL_CONF:
                macIndex += 1;
                break;
            default:
...
LoRaMacStatus_t LoRaMacMibSetRequestConfirm( MibRequestConfirm_t *mibSet )
{
    ...
    switch( mibSet->Type )
    {
        case MIB_DEVICE_CLASS:
        {
            if (LoRaMacDeviceClass != mibSet->Param.Class) {
                switch( mibSet->Param.Class )
                {
                    case CLASS_A:
                    {
                        LoRaMacDeviceClass = CLASS_A;
                    ...
                    case CLASS_B:
                    {
                        LoRaMacDeviceClass = CLASS_B;
                        break;
                    }
                    case CLASS_C:
                    {
                      
                       status = AddMacCommand(MOTE_MAC_DEVICE_MODEL_IND, CLASS_C, 0);
                       if (status == LORAMAC_STATUS_OK) {
