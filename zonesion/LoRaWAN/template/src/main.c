    ...
    case 2:
        switch( region )
        {
            ...
            case LORAMAC_REGION_KR920:
            {
                ...
                i += 1;
                sprintf(AppData, "{A0=%d}", i);
                AppDataSizeBackup = AppDataSize = strlen(AppData);
                break;
            }
...
uint8_t BoardGetRandomSeed(void)
{
    return 10;
}
...
TimerTime_t RtcGetAdjustedTimeoutValue(uint32_t t)
{
  printf("RtcGetAdjustedTimeoutValue(%u)\r\n", t);
  return 0;
}
void RtcSetTimeout(uint32_t v)
{
  printf("RtcSetTimeout(%u)\r\n", v);
}
TimerTime_t RtcGetElapsedAlarmTime(void)
{
  printf("RtcGetElapsedAlarmTime()\r\n");
  return 0;
}
TimerTime_t RtcComputeElapsedTime(TimerTime_t et)
{
  printf("RtcComputeElapsedTime(%u)\r\n", et);
  return 0;
}
...
int GetBoardPowerSource(void)
{
  return 1; //BATTERY_POWER //USB_POWER;
}
#define Debug      printf
/**
...
#if defined(WITH_xLab) && defined(WITH_xLab_AP) || !defined(WITH_xLab)
   uart2_init(38400);
    ...
    while( 1 )
    {
      ...
      TimerProcess();
        ...
        switch( DeviceState )
        {
            ...
            {
               uint16_t m[6] = {0x0001,0,0,0,0,0};
                ChanMaskSetParams_t cmp;
                ...
                mibReq.Type = MIB_ADR;
                mibReq.Param.AdrEnable = 0;//LORAWAN_ADR_ON;
                LoRaMacMibSetRequestConfirm( &mibReq );
