...
int sx1278Rssi2()
{
  return SX1276ReadBuffer(REG_LR_RSSIVALUE);
}
...
static void  SX1276LoRaSetRFFrequency(unsigned int bf) {
    unsigned int val = (unsigned int)(bf / 61.035162f); //(434000000.0f / 0x6c8000)//7110656
...
**------------------------------------------------------------------------------------------------------
********************************************************************************************************/
void  SX1276LoRaSetMobileNode(int enable) {
	unsigned char RECVER_DAT;
...
void  sx1278SendPacket(unsigned char *pkt,unsigned char len) {
	...
	//SX1276WriteBuffer( REG_LR_FIFOTXBASEADDR, 0x80); //reset tx fifo
	SX1276WriteBuffer( REG_LR_FIFOADDRPTR, 0x80);
...
void sx1278Init(void)
{
   ...
   /*     
	...
	SX1276WriteBuffer(REG_LR_IRQFLAGSMASK, 0); 
        ...
        SX1276WriteBuffer( REG_LR_FIFOTXBASEADDR, 0x80);
        SX1276WriteBuffer(REG_LR_FIFORXBASEADDR, 0x00);
        SX1276WriteBuffer(REG_LR_FIFOADDRPTR, 0x00);
        SX1276WriteBuffer( 0x39 , 0x34);
        SX1276LoRaSetPreambleLength(8);
        
        SX1276WriteBuffer( 0x31 , 0x03);
        SX1276WriteBuffer( 0x37 , 0x0A);
...
static void RxChainCalibration( void )
{
    ...
    // Save context
    regPaConfigInitVal = SX1276ReadBuffer( REG_PACONFIG );
    initialFreq = ( double )( ( ( uint32_t )SX1276ReadBuffer( REG_FRFMSB ) << 16 ) |
                              ( ( uint32_t )SX1276ReadBuffer( REG_FRFMID ) << 8 ) |
                              ( ( uint32_t )SX1276ReadBuffer( REG_FRFLSB ) ) ) * ( double )FREQ_STEP;

    // Cut the PA just in case, RFO output, power = -1 dBm
    SX1276WriteBuffer( REG_PACONFIG, 0x00 );
    ...
    // Restore context
    SX1276WriteBuffer( REG_PACONFIG, regPaConfigInitVal );
    SX1276LoRaSetRFFrequency(initialFreq);
}

void invertiq_tx(int st)
{
  if (st != 0) {
...
int sx1278RxInProgress(void)
{
  ...
  return !!SX1276ReadBuffer(REG_LR_IRQFLAGS) & IRQ_LORA_VALID_HEADER;
}
